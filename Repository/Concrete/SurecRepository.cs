...
public class SurecRepository(VarlikEnvanteriDbContext context)
    : GenericRepository<Surec, int>(context), ISurecRepository
{
    // Surec'e özel metodların implementasyonları buraya eklenebilir
    public async Task<List<ListSurecDto>> GetListWithDetailsAsync(
        Expression<Func<ListSurecDto, bool>>? filter = null)
    {
        var query = context.SurecDetay.AsQueryable();
    ...
    public async Task<DataTablesResponse<ListSurecDto>> ProcessTableRequestAsync(DataTablesRequest request,
        Expression<Func<ListSurecDto, bool>>? filter = null)
    {
        var query = context.SurecDetay.AsQueryable();
