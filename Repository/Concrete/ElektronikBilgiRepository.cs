...
public class ElektronikBilgiRepository(VarlikEnvanteriDbContext context)
    : GenericRepository<ElektronikBilgi, int>(context), IElektronikBilgiRepository
{
    // ElektronikBilgi'ye özel metodların implementasyonları buraya eklenebilir
    public async Task<List<ListElektronikBilgiDto>> GetListWithDetailsAsync(
        Expression<Func<ListElektronikBilgiDto, bool>>? filter = null)
    {
        var query = context.ElektronikBilgiDetay.AsQueryable();
    ...
    public async Task<DataTablesResponse<ListElektronikBilgiDto>> ProcessTableRequestAsync(DataTablesRequest request,
        Expression<Func<ListElektronikBilgiDto, bool>>? filter = null)
    {
        var query = context.ElektronikBilgiDetay.AsQueryable();
