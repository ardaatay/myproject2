...
public class IoTCihazRepository(VarlikEnvanteriDbContext context)
    : GenericRepository<IoTCihaz, int>(context), IIoTCihazRepository
{
    public async Task<List<ListIoTCihazDto>> GetListWithDetailsAsync(
        Expression<Func<ListIoTCihazDto, bool>>? filter = null)
    {
        var query = context.IoTCihazDetay.AsQueryable();
    ...
    public async Task<DataTablesResponse<ListIoTCihazDto>> ProcessTableRequestAsync(
    ...
    public async Task<List<RaporAnasayfa>> GetRaporIoTCihazlarAsync()
    {
        var ioTCihazlar = context.IoTCihazlari.OrderByDescending(a => a.EnvantereGirisTarihi).Take(3)
        ...
        return await ioTCihazlar.ToListAsync();
    }
}
