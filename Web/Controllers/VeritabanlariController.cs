...
{
    [Authorize]
    public class VeritabanlariController(
        ...
        : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> GetVeritabanlaris(int varlikSahibi = 0, int varlikSahibiAltDepartman = 0, int durumId = 0)
        {
            ...
            if (durumId != 0)
                request.FilterBag.WithFilter<ListVeritabaniDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
                request.FilterBag.WithFilter<ListVeritabaniDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                request.FilterBag.WithFilter<ListVeritabaniDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);


            var response = await veritabaniService.GetAllAsync(request);
        ...
        public async Task<IActionResult> Create()
        {
            ...
            return View(new CreateVeritabaniDto());
        }

        [Authorize(Policy = Yetkiler.BilgiVarlikOlustur)]
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Create(CreateVeritabaniDto dto)
        {
            if (ModelState.IsValid)
            {
                await veritabaniService.AddAsync(dto);
        ...
        {
            var dto = await veritabaniService.GetByIdAsync(id);
            if (dto.Id == 0)
                return NotFound();
        ...
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Edit(UpdateVeritabaniDto dto)
        {
            if (ModelState.IsValid)
            {
                await veritabaniService.UpdateAsync(dto);
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> Copy(int id)
        {
            ...
            var newDto = mapper.Map<CreateVeritabaniDto>(dto);
        ...
        {
            await veritabaniService.DeleteAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            await veritabaniService.DeleteDatabaseAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            ViewBag.Kategoriler = new SelectList(await kategoriService.GetAllByUstIdAsync(10), "Id", "Ad");
        ...
        public async Task<IActionResult> DownloadExcel(string search, int varlikSahibi, int varlikSahibiAltDepartman, int durumId)
        {
            ...
            if (durumId != 0)
                filterBag.WithFilter<ListVeritabaniDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
                filterBag.WithFilter<ListVeritabaniDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                filterBag.WithFilter<ListVeritabaniDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);


            var stream = await excelService.GenerateExcelVeritabani(search, filterBag);
