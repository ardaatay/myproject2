...
[Authorize]
public class AgveSistemlerController(
    ...
    : Controller
{
    ...
    [HttpPost]
    public async Task<IActionResult> GetAgveSistems(int varlikSahibi = 0, int varlikSahibiAltDepartman = 0, int durumId = 0)
    {
        ...
        if (durumId != 0)
            request.FilterBag.WithFilter<ListAgveSistemDto>(x => x.DurumId == durumId);

        if (varlikSahibi != 0)
            request.FilterBag.WithFilter<ListAgveSistemDto>(x => x.VarlikSahibiId == varlikSahibi);

        if (varlikSahibiAltDepartman != 0)
            request.FilterBag.WithFilter<ListAgveSistemDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);

        var response = await agveSistemService.GetAllAsync(request);
    ...
    public async Task<IActionResult> Create()
    {
        ...
        return View(new CreateAgveSistemDto());
    }

    [Authorize(Policy = Yetkiler.TeknikVarlikOlustur)]
    [HttpPost]
    [GuvenlikModuKontrol]
    public async Task<IActionResult> Create(CreateAgveSistemDto dto)
    {
        if (ModelState.IsValid)
        {
            await agveSistemService.AddAsync(dto);
    ...
    {
        var dto = await agveSistemService.GetByIdAsync(id);
        if (dto.Id == 0)
            return NotFound();
    ...
    [HttpPost]
    [GuvenlikModuKontrol]
    public async Task<IActionResult> Edit(UpdateAgveSistemDto dto)
    {
        if (ModelState.IsValid)
        {
            await agveSistemService.UpdateAsync(dto);
            return RedirectToAction(nameof(Index));
        }
    ...
    public async Task<IActionResult> Copy(int id)
    {
        ...
        var newDto = mapper.Map<CreateAgveSistemDto>(dto);
    ...
    {
        await agveSistemService.DeleteAsync(id);
        return RedirectToAction(nameof(Index));
    }
    ...
    {
        await agveSistemService.DeleteDatabaseAsync(id);
        return RedirectToAction(nameof(Index));
    }
    ...
    {
        ViewBag.Kategoriler = new SelectList(await kategoriService.GetAllByUstIdAsync(1), "Id", "Ad");
        ...
        ViewBag.LisansTakipSorumlulari = new SelectList(await lisansTakipSorumlusuService.GetAllAsync(), "Id", "Ad");

        var birimId = varlikSahibiId ?? Convert.ToInt32(User.Claims.FirstOrDefault(c => c.Type == "BirimId")?.Value);
        ...
        ViewBag.OperasyonelBirimler = new SelectList(await birimService.GetUstBirimlerAsync(), "Id", "Ad");
    }
    ...
    // {
    //     var stream = await excelService.GenerateExcelAgveSistem();
    //     return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "AgveSistemler.xlsx");
    // }

    [Authorize]
    [HttpGet]
    public async Task<IActionResult> DownloadExcel(string search, int varlikSahibi, int varlikSahibiAltDepartman, int durumId = 0)
    {
        FilterBag filterBag = new FilterBag();
        if ( varlikSahibi != 0)
            filterBag.WithFilter<ListAgveSistemDto>(x => x.VarlikSahibiId == varlikSahibi);
        if (varlikSahibiAltDepartman != 0)
            filterBag.WithFilter<ListAgveSistemDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);
        if (durumId != 0)
            filterBag.WithFilter<ListAgveSistemDto>(x => x.DurumId == durumId);

        var stream = await excelService.GenerateExcelAgveSistem(search, filterBag);
        return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "AgveSistemler.xlsx");
    }
}
