...
{
    [Authorize]
    public class UygulamalarController(
        ...
        : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> GetUygulamalars(int varlikSahibi = 0, int varlikSahibiAltDepartman = 0, int durumId = 0)
        {
            ...
            if (durumId != 0)
                request.FilterBag.WithFilter<ListUygulamaDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
                request.FilterBag.WithFilter<ListUygulamaDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                request.FilterBag.WithFilter<ListUygulamaDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);

            var response = await uygulamaService.GetAllAsync(request);
        ...
        public async Task<IActionResult> Create()
        {
            ...
            return View(new CreateUygulamaDto());
        }

        [Authorize(Policy = Yetkiler.TeknikVarlikOlustur)]
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Create(CreateUygulamaDto dto)
        {
            if (ModelState.IsValid)
            {
                await uygulamaService.AddAsync(dto);
        ...
        {
            var dto = await uygulamaService.GetByIdAsync(id);
            if (dto.Id == 0)
                return NotFound();
        ...
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Edit(UpdateUygulamaDto dto)
        {
            if (ModelState.IsValid)
            {
                await uygulamaService.UpdateAsync(dto);
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> Copy(int id)
        {
            ...
            var newDto = mapper.Map<CreateUygulamaDto>(dto);
        ...
        {
            await uygulamaService.DeleteAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            await uygulamaService.DeleteDatabaseAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            ViewBag.Kategoriler = new SelectList(await kategoriService.GetAllByUstIdAsync(2), "Id", "Ad");
        ...
        public async Task<IActionResult> DownloadExcel(string search, int varlikSahibi, int varlikSahibiAltDepartman, int durumId)
        {
            ...
            if (durumId != 0)
                filterBag.WithFilter<ListUygulamaDto>(x => x.DurumId == durumId);
            if (varlikSahibi != 0)
                filterBag.WithFilter<ListUygulamaDto>(x => x.VarlikSahibiId == varlikSahibi);
            if (varlikSahibiAltDepartman != 0)
                filterBag.WithFilter<ListUygulamaDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);

            var stream = await excelService.GenerateExcelUygulama(search, filterBag);
