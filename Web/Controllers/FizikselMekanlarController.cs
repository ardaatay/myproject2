...
{
    [Authorize]
    public class FizikselMekanlarController(
        ...
        : Controller
    {
        ...
        public async Task<IActionResult> GetFizikselMekanlars(int varlikSahibi = 0, int varlikSahibiAltDepartman = 0, int durumId = 0)
        {
            ...
            if (durumId != 0)
                request.FilterBag.WithFilter<ListFizikselMekanDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
                request.FilterBag.WithFilter<ListFizikselMekanDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                request.FilterBag.WithFilter<ListFizikselMekanDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);

            // Kolon adlarını veritabanı alan adlarıyla eşleştir

            var response = await fizikselMekanService.GetAllAsync(request);
        ...
        public async Task<IActionResult> Create()
        {
            ...
            return View(new CreateFizikselMekanDto());
        }

        [Authorize(Policy = Yetkiler.TeknikVarlikOlustur)]
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Create(CreateFizikselMekanDto dto)
        {
            if (ModelState.IsValid)
            {
                await fizikselMekanService.AddAsync(dto);
        ...
        {
            var dto = await fizikselMekanService.GetByIdAsync(id);
            if (dto.Id == 0)
                return NotFound();
        ...
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Edit(UpdateFizikselMekanDto dto)
        {
            if (ModelState.IsValid)
            {
                await fizikselMekanService.UpdateAsync(dto);
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> Copy(int id)
        {
            ...
            var newDto = mapper.Map<CreateFizikselMekanDto>(dto);
        ...
        {
            await fizikselMekanService.DeleteAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            await fizikselMekanService.DeleteDatabaseAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            ViewBag.Kategoriler = new SelectList(await kategoriService.GetAllByUstIdAsync(5), "Id", "Ad");
        ...
        public async Task<IActionResult> DownloadExcel(string search, int varlikSahibi, int varlikSahibiAltDepartman, int durumId)
        {
            ...
            if (durumId != 0)
                filterBag.WithFilter<ListFizikselMekanDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
                filterBag.WithFilter<ListFizikselMekanDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                filterBag.WithFilter<ListFizikselMekanDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);


            var stream = await excelService.GenerateExcelFizikselMekan(search, filterBag);
            return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                "FizikselMekanlar.xlsx");
        }
    }
}
