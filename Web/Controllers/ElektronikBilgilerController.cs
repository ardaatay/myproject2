...
{
    [Authorize]
    public class ElektronikBilgilerController(
        ...
        : Controller
    {
        [Authorize(Policy = Yetkiler.BilgiVarlikListele)]
        ...
        [HttpPost]
        public async Task<IActionResult> GetElektronikBilgilers(int varlikSahibi = 0, int varlikSahibiAltDepartman = 0, int durumId = 0)
        {
            ...
            if (durumId != 0)
                request.FilterBag.WithFilter<ListElektronikBilgiDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
                request.FilterBag.WithFilter<ListElektronikBilgiDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                request.FilterBag.WithFilter<ListElektronikBilgiDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);


            var response = await elektronikBilgiService.GetAllAsync(request);

            return Json(response);
        }

        [Authorize(Policy = Yetkiler.BilgiVarlikOlustur)]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Create()
        {
            await LoadSelectLists();
            return View(new CreateElektronikBilgiDto());
        }

        [Authorize(Policy = Yetkiler.BilgiVarlikOlustur)]
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Create(CreateElektronikBilgiDto dto)
        {
            if (ModelState.IsValid)
            {
                await elektronikBilgiService.AddAsync(dto);
        ...
        [Authorize(Policy = Yetkiler.BilgiVarlikDuzenle)]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Edit(int id)
        {
            var dto = await elektronikBilgiService.GetByIdAsync(id);
            if (dto.Id == 0)
                return NotFound();
        ...
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Edit(UpdateElektronikBilgiDto dto)
        {
            if (ModelState.IsValid)
            {
                await elektronikBilgiService.UpdateAsync(dto);
                return RedirectToAction(nameof(Index));
            }
        ...
        [Authorize(Policy = Yetkiler.BilgiVarlikGoruntule)]
        ...
        public async Task<IActionResult> Copy(int id)
        {
            ...
            var newDto = mapper.Map<CreateElektronikBilgiDto>(dto);
        ...
        {
            await elektronikBilgiService.DeleteAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            await elektronikBilgiService.DeleteDatabaseAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            ViewBag.Kategoriler = new SelectList(await kategoriService.GetAllByUstIdAsync(8), "Id", "Ad");
        ...
        public async Task<IActionResult> DownloadExcel(string search, int varlikSahibi, int varlikSahibiAltDepartman, int durumId)
        {
            ...
            if (durumId != 0)
                filterBag.WithFilter<ListElektronikBilgiDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
                filterBag.WithFilter<ListElektronikBilgiDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                filterBag.WithFilter<ListElektronikBilgiDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);


            var stream = await excelService.GenerateExcelElektronikBilgi(search, filterBag);
            return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                "ElektronikBilgiler.xlsx");
        }
    }
}
