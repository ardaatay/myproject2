...
{
    [Authorize]
    public class SureclerController(
        ...
        : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> GetSureclers(int varlikSahibi = 0, int varlikSahibiAltDepartman = 0, int durumId = 0)
        {
            ...
            if (durumId != 0)
                request.FilterBag.WithFilter<ListSurecDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
                request.FilterBag.WithFilter<ListSurecDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                request.FilterBag.WithFilter<ListSurecDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);


            var response = await surecService.GetAllAsync(request);
        ...
        public async Task<IActionResult> Create()
        {
            ...
            return View(new CreateSurecDto());
        }

        [Authorize(Policy = Yetkiler.BilgiVarlikOlustur)]
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Create(CreateSurecDto dto)
        {
            if (ModelState.IsValid)
            {
                await surecService.AddAsync(dto);
        ...
        {
            var dto = await surecService.GetByIdAsync(id);
            if (dto.Id == 0)
                return NotFound();
        ...
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Edit(UpdateSurecDto dto)
        {
            if (ModelState.IsValid)
            {
                await surecService.UpdateAsync(dto);
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> Copy(int id)
        {
            ...
            var newDto = mapper.Map<CreateSurecDto>(dto);
        ...
        {
            await surecService.DeleteAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            await surecService.DeleteDatabaseAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            ViewBag.Kategoriler = new SelectList(await kategoriService.GetAllByUstIdAsync(9), "Id", "Ad");
        ...
        public async Task<IActionResult> DownloadExcel(string search, int varlikSahibi, int varlikSahibiAltDepartman, int durumId)
        {
            ...
            if (durumId != 0)
               filterBag.WithFilter<ListSurecDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
               filterBag.WithFilter<ListSurecDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                filterBag.WithFilter<ListSurecDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);


            var stream = await excelService.GenerateExcelSurec(search, filterBag);
