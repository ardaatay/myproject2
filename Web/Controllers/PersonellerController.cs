...
{
    [Authorize]
    public class PersonellerController(
        ...
        : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> GetPersonellers(int varlikSahibi = 0, int varlikSahibiAltDepartman = 0, int durumId = 0)
        {
            ...
            if (durumId != 0)
                request.FilterBag.WithFilter<ListPersonelDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
                request.FilterBag.WithFilter<ListPersonelDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                request.FilterBag.WithFilter<ListPersonelDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);


            // Kolon adlarını veritabanı alan adlarıyla eşleştir

            var response = await personelService.GetAllAsync(request);
        ...
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Create(CreatePersonelDto dto)
        {
            if (ModelState.IsValid)
            {
                await personelService.AddAsync(dto);
        ...
        {
            var dto = await personelService.GetByIdAsync(id);
            if (dto.Id == 0)
                return NotFound();
        ...
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Edit(UpdatePersonelDto dto)
        {
            if (ModelState.IsValid)
            {
                await personelService.UpdateAsync(dto);
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> Copy(int id)
        {
            ...
            var newDto = mapper.Map<CreatePersonelDto>(dto);
        ...
        {
            await personelService.DeleteDatabaseAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            ViewBag.Kategoriler = new SelectList(await kategoriService.GetAllByUstIdAsync(6), "Id", "Ad");
        ...
        public async Task<IActionResult> DownloadExcel(string search, int varlikSahibi, int varlikSahibiAltDepartman, int durumId)
        {
            ...
            if (durumId != 0)
                filterBag.WithFilter<ListPersonelDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
                filterBag.WithFilter<ListPersonelDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                filterBag.WithFilter<ListPersonelDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);


            var stream = await excelService.GenerateExcelPersonel(search, filterBag);
