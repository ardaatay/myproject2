...
{
    [Authorize]
    public class TasinabilirCihazveOrtamlarController(
        ...
        : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> GetTasinabilirCihazveOrtamlars(int varlikSahibi = 0,
            int varlikSahibiAltDepartman = 0, int durumId = 0)
        {
            ...
            if (durumId != 0)
                request.FilterBag.WithFilter<ListTasinabilirCihazveOrtamDto>(x => x.DurumId == durumId);
            if (varlikSahibi != 0)
                request.FilterBag.WithFilter<ListTasinabilirCihazveOrtamDto>(x => x.VarlikSahibiId == varlikSahibi);
            if (varlikSahibiAltDepartman != 0)
                request.FilterBag.WithFilter<ListTasinabilirCihazveOrtamDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);

            var response =
                await tasinabilirCihazveOrtamService.GetAllAsync(request);
        ...
        public async Task<IActionResult> Create()
        {
            ...
            return View(new CreateTasinabilirCihazveOrtamDto());
        }

        [Authorize(Policy = Yetkiler.TeknikVarlikOlustur)]
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Create(CreateTasinabilirCihazveOrtamDto dto)
        {
            if (ModelState.IsValid)
            {
                await tasinabilirCihazveOrtamService.AddAsync(dto);
        ...
        {
            var dto = await tasinabilirCihazveOrtamService.GetByIdAsync(id);
            if (dto.Id == 0)
                return NotFound();
        ...
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Edit(UpdateTasinabilirCihazveOrtamDto dto)
        {
            if (ModelState.IsValid)
            {
                await tasinabilirCihazveOrtamService.UpdateAsync(dto);
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> Copy(int id)
        {
            ...
            var newDto = mapper.Map<CreateTasinabilirCihazveOrtamDto>(dto);
        ...
        {
            await tasinabilirCihazveOrtamService.DeleteAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            await tasinabilirCihazveOrtamService.DeleteDatabaseAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            ViewBag.Kategoriler = new SelectList(await kategoriService.GetAllByUstIdAsync(3), "Id", "Ad");
        ...
        public async Task<IActionResult> DownloadExcel(string search, int varlikSahibi, int varlikSahibiAltDepartman, int durumId)
        {
            ...
            if (durumId != 0)
                filterBag.WithFilter<ListTasinabilirCihazveOrtamDto>(x => x.DurumId == durumId);
            if (varlikSahibi != 0)
                filterBag.WithFilter<ListTasinabilirCihazveOrtamDto>(x => x.VarlikSahibiId == varlikSahibi);
            if (varlikSahibiAltDepartman != 0)
                filterBag.WithFilter<ListTasinabilirCihazveOrtamDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);

            var stream =
                await excelService.GenerateExcelTasinabilirCihaz(search, filterBag);
            return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                "TasinabilirCihazveOrtam.xlsx");
        }
    }
}
