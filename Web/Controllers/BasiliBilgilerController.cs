...
{
    [Authorize]
    public class BasiliBilgilerController(
        ...
        : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> GetBasiliBilgilers(int varlikSahibi = 0, int varlikSahibiAltDepartman = 0, int durumId = 0)
        {
            ...
            if (durumId != 0)
                request.FilterBag.WithFilter<ListBasiliBilgiDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
                request.FilterBag.WithFilter<ListBasiliBilgiDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                request.FilterBag.WithFilter<ListBasiliBilgiDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);


            // Kolon adlarını veritabanı alan adlarıyla eşleştir

            var response = await basiliBilgiService.GetAllAsync(request);
        ...
        public async Task<IActionResult> Create()
        {
            ...
            return View(new CreateBasiliBilgiDto());
        }

        [Authorize(Policy = Yetkiler.BilgiVarlikOlustur)]
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Create(CreateBasiliBilgiDto dto)
        {
            if (ModelState.IsValid)
            {
                await basiliBilgiService.AddAsync(dto);
        ...
        {
            var dto = await basiliBilgiService.GetByIdAsync(id);
            if (dto.Id == 0)
                return NotFound();
        ...
        [HttpPost]
        [GuvenlikModuKontrol]
        public async Task<IActionResult> Edit(UpdateBasiliBilgiDto dto)
        {
            if (ModelState.IsValid)
            {
                await basiliBilgiService.UpdateAsync(dto);
                return RedirectToAction(nameof(Index));
            }
        ...
        public async Task<IActionResult> Copy(int id)
        {
            ...
            var newDto = mapper.Map<CreateBasiliBilgiDto>(dto);
        ...
        {
            await basiliBilgiService.DeleteAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            await basiliBilgiService.DeleteDatabaseAsync(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            ViewBag.Kategoriler = new SelectList(await kategoriService.GetAllByUstIdAsync(7), "Id", "Ad");
        ...
        public async Task<IActionResult> DownloadExcel(string search, int varlikSahibi, int varlikSahibiAltDepartman, int durumId)
        {
            ...
            if (durumId != 0)
               filterBag.WithFilter<ListBasiliBilgiDto>(x => x.DurumId == durumId);

            if (varlikSahibi != 0)
                filterBag.WithFilter<ListBasiliBilgiDto>(x => x.VarlikSahibiId == varlikSahibi);

            if (varlikSahibiAltDepartman != 0)
                filterBag.WithFilter<ListBasiliBilgiDto>(x => x.VarlikSahibiAltDepartmanId == varlikSahibiAltDepartman);


            var stream = await excelService.GenerateExcelBasiliBilgi(search, filterBag);
